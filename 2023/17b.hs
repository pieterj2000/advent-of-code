...
neighbours g (Node p dir) = nodes
    where   possDirs = filter (\d -> (dir /= d) && not (opposites d dir)) [U,D,L,R]
            (_,dest) = A.bounds g
            possPaths = (filter (\(p,d) -> last p == dest) [ (moveStraightThenDir p dir (straight - 1) dir, dir) | straight<-[3..9] ] )
                        ++ [ (moveStraightThenDir p dir straight d, d) | straight<-[3..9], d<-possDirs]
