...
--calc :: String -> Int
--calc = getButtonCount . until rxSentCheck (doAllSignals . pressButton) . buildDing . parseInput
--calc = (\(_,s,_) -> map (s !) [46, 21, 52, 19, 42, 51, 23, 50, 37, 44, 40, 27, 34, 30]) . nTimes 3944 (doAllSignals . pressButton) . buildDing . parseInput
--calc = (\(_,s,_) -> V.indexed s) . nTimes 1 (doAllSignals . pressButton) . buildDing . parseInput

calc _ =
    let a = sum $ map (\i -> 2^(i-1)) [1,2,3,6,7,9,10,11,12]
        b = sum $ map (\i -> 2^(i-1)) [1,2,3,6,8,9,10,11,12]
        c = sum $ map (\i -> 2^(i-1)) [1,2,4,6,7,9,10,11,12]
        d = sum $ map (\i -> 2^(i-1)) [1,2,5,6,8,9,10,11,12]
    in foldr1 lcm [a,b,c,d] :: Int
...
rxSentCheck :: State -> Bool
rxSentCheck (_, _, queue) = rxSent queue

getButtonCount :: State -> Int
getButtonCount (_,_, Queue _ _ _ _ c) = c

calcScore :: State -> Int
calcScore (_,_,Queue _ cH cL rx c) = cH*cL
...
----------
data Queue = Queue { queue :: S.Seq (Int, Signal, Int), countHigh :: Int, countLow :: Int, rxSent :: Bool, buttonCount :: Int } deriving (Show)
emptyQueue :: Queue
emptyQueue = Queue S.empty 0 0 False 0

--enqueue :: Queue -> (Int, Signal, Int) -> Queue
--enqueue (Queue s cH cL rx c) (_,High,-1)  = Queue s (cH+1) cL rx c
--enqueue (Queue s cH cL rx c) (_,Low,-1)  = Queue s cH (cL+1) True c
--enqueue (Queue s cH cL rx c) (src,High,dest) = Queue (s :|> (src,High,dest)) (cH+1) cL rx (if src == (-1) then c + 1 else c)
--enqueue (Queue s cH cL rx c) (src,Low,dest) = Queue (s :|> (src,Low,dest)) cH (cL+1) rx (if src == (-1) then c + 1 else c)

enqueue :: Queue -> (Int, Signal, Int) -> Queue
enqueue (Queue s cH cL rx c) (_,High,-1)  = Queue s (cH+1) cL rx c
enqueue (Queue s cH cL rx c) (_,Low,-1)  = Queue s cH (cL+1) rx c
enqueue (Queue s cH cL rx c) (_,High,46)  = Queue s (cH+1) cL rx c
enqueue (Queue s cH cL rx c) (_,Low,46)  = Queue s cH (cL+1) True c
enqueue (Queue s cH cL rx c) (src,High,dest) = Queue (s :|> (src,High,dest)) (cH+1) cL rx (if src == (-1) then c + 1 else c)
enqueue (Queue s cH cL rx c) (src,Low,dest) = Queue (s :|> (src,Low,dest)) cH (cL+1) rx (if src == (-1) then c + 1 else c)

dequeue :: Queue -> ((Int, Signal, Int), Queue)
dequeue q@(Queue s cH cL rx c)
    | isEmpty q = error "dequeueing empty queue"
    | otherwise = let (a :< rest) = S.viewl s in (a, Queue rest cH cL rx c)

isEmpty :: Queue -> Bool
isEmpty (Queue s cH cL rx c) = S.null s
