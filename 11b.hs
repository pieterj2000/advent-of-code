...
doRows = snd . mapAccumL (\(xOld, skip) (x,y) -> let newSkip = skip + max (x-xOld-1) 0 in ( (x, newSkip), (x + newSkip*999999,y) ) ) (0,0)

doCols :: [Point] -> [Point]
-- accumulator is (yOld, skip), new acc is (y, skip + max (y - yOld - 1, 0) )
doCols = snd . mapAccumL (\(yOld, skip) (x,y) -> let newSkip = skip + max (y-yOld-1) 0 in ( (y, newSkip), (x,y + newSkip*999999) ) ) (0,0)
